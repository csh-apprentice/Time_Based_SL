...
enum { XLO = 0, XHI = 1, YLO = 2, YHI = 3, ZLO = 4, ZHI = 5, SOUT=6, SIN=7 };
enum { NONE = 0, EDGE, CONSTANT, VARIABLE };

static const char *wallpos[] = {"xlo", "xhi", "ylo", "yhi", "zlo", "zhi","sphereout","spherein"};
...
FixWall::FixWall(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg), nwall(0)
{
  ...
  for (int i = 0; i < 8; i++) xstr[i] = estr[i] = sstr[i] = lstr[i] = fstr[i] = kstr[i] = nullptr;
  ...
  while (iarg < narg) {
    ...
    if ((strcmp(arg[iarg], "xlo") == 0) || (strcmp(arg[iarg], "xhi") == 0) ||
        ...
        (strcmp(arg[iarg], "zlo") == 0) || (strcmp(arg[iarg], "zhi") == 0) ||
         (strcmp(arg[iarg], "sphereout") == 0) || (strcmp(arg[iarg], "spherein") == 0)
        ) {
      ...
      if (strcmp(arg[iarg], "xlo") == 0) {
        ...
        newwall = ZHI;
      } else if (strcmp(arg[iarg], "sphereout") == 0) {
        newwall = SOUT;
      } else if (strcmp(arg[iarg], "spherein") == 0) {
        newwall = SIN;
      }
    ...
    /**
    //Code EXTENSION FOR SPHERE
    
    else if ((strcmp(arg[iarg], "spherein") == 0) || (strcmp(arg[iarg], "sphereout") == 0)) {
      if (iarg + wantargs > narg) error->all(FLERR, "Missing argument for fix {} command", style);

      int newwall;
      if (strcmp(arg[iarg], "spherein") == 0) {
        newwall = SIN;
      } else if (strcmp(arg[iarg], "sphereout") == 0) {
        newwall = SOUT;
      }
      ...
      // 2023/10/13 ENDS
      if (strcmp(arg[iarg + 1], "EDGE") == 0) {
      error->all(FLERR, "COUld not use EDGE in fix wallsphere!");
      } else if (utils::strmatch(arg[iarg + 1], "^v_")) {
        xstyle[nwall] = VARIABLE;
        xstr[nwall] = utils::strdup(arg[iarg + 1] + 2);
      } else {
  ...
  for (int m = 0; m < nwall; m++)
    if ((wallwhich[m] == ZLO || wallwhich[m] == ZHI ||wallwhich[m]==SIN ||wallwhich[m]==SOUT) && domain->dimension == 2)
      error->all(FLERR, "Cannot use fix {} zlo/zhi/spherein/sphereout for a 2d simulation", style);
