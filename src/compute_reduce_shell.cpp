...
#include "compute_reduce_shell.h"
...
ComputeReduceShell::ComputeReduceShell(LAMMPS *lmp, int narg, char **arg) :
...
double ComputeReduceShell::compute_one(int m, int flag)
{
  //region->prematch();
...
bigint ComputeReduceShell::count(int m)
{
  auto &val = values[m];
