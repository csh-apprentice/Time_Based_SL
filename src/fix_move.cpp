...
enum { LINEAR, WIGGLE, ROTATE, VARIABLE, TRANSROT, SPHERE };
enum { EQUAL, ATOM };
...
FixMove::FixMove(LAMMPS *lmp, int narg, char **arg) :
    ...
    qoriginal(nullptr), displace(nullptr), velocity(nullptr), rvarstr(nullptr), vrvarstr(nullptr),
    rdisplace(nullptr), rvelocity(nullptr)
{
  ...
  if (strcmp(arg[3], "linear") == 0) {
    ...
    if (narg < 6) error->all(FLERR, "Illegal fix move command");
    iarg = 6;
    mstyle = SPHERE;
    if (strcmp(arg[4], "NULL") == 0)
      rvarstr = nullptr;
    else if (utils::strmatch(arg[4], "^v_")) {
      rvarstr = utils::strdup(arg[4] + 2);
    } else
      error->all(FLERR, "Illegal fix move command");
    if (strcmp(arg[5], "NULL") == 0)
      vrvarstr = nullptr;
    else if (utils::strmatch(arg[5], "^v_")) {
      vrvarstr = utils::strdup(arg[5] + 2);
    } else
      error->all(FLERR, "Illegal fix move command");
  }
  ...
  if (domain->dimension == 2) {
    ...
    if (((mstyle == SPHERE)))
      error->all(FLERR, "Fix move cannot set sphere motion for 2d problem");
  }
...
FixMove::~FixMove()
{
  ...
  memory->destroy(rdisplace);
  memory->destroy(rvelocity);
...
void FixMove::init()
{
  ...
  /*Extension code for sphere*/
  if (mstyle == SPHERE)
  {
    if (rvarstr) {
      rvar = input->variable->find(rvarstr);
      if (rvar < 0) error->all(FLERR, "Variable name for fix move does not exist");
      if (input->variable->equalstyle(rvar))
        rvarstyle = EQUAL;
      else if (input->variable->atomstyle(rvar))
        rvarstyle = ATOM;
      else
        error->all(FLERR, "Variable for fix move is invalid style");
    }
    if (vrvarstr) {
      vrvar = input->variable->find(vrvarstr);
      if (vrvar < 0) error->all(FLERR, "Variable name for fix move does not exist");
      if (input->variable->equalstyle(vrvar))
        vrvarstyle = EQUAL;
      else if (input->variable->atomstyle(vrvar))
        vrvarstyle = ATOM;
      else
        error->all(FLERR, "Variable for fix move is invalid style");
    }
    if (rvarstr && rvarstyle == ATOM) rdisplaceflag = 1;
    if (vrvarstr && vrvarstyle == ATOM) rvelocityflag = 1;
  ...
  if (rdisplaceflag)
    memory->create(rdisplace, maxatom,  "move:displace");
  else
    rdisplace = nullptr;
  if (rvelocityflag)
    memory->create(rvelocity, maxatom,  "move:velocity");
  else
    rvelocity = nullptr;
...
void FixMove::initial_integrate(int /*vflag*/)
{
  ...
  /*Extension for sphere*/
  else if (mstyle == SPHERE) {

    // reallocate displace and velocity arrays as necessary

    if ((rdisplaceflag || rvelocityflag) && atom->nmax > maxatom) {
      maxatom = atom->nmax;
      if (rdisplaceflag) {
        memory->destroy(rdisplace);
        memory->create(rdisplace, maxatom, "move:rdisplace");
      }
      if (rvelocityflag) {
        memory->destroy(rvelocity);
        memory->create(rvelocity, maxatom,  "move:rvelocity");
      }
    }
    //1158
    // pre-compute variable values, wrap with clear/add
    ...
    if (rvarstr) {
      if (rvarstyle == EQUAL)
        double dr=input->variable->compute_equal(rvar);
      else
        input->variable->compute_atom(rvar, igroup, &rdisplace[0], 1, 0);
    }
    if (vrvarstr) {
      if (vrvarstyle == EQUAL)
        vr = input->variable->compute_equal(vrvar);
      else
        input->variable->compute_atom(vrvar, igroup, &rvelocity[0], 1, 0);
    }
    ...
    for (int i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit) {
        ...
        xold[1] = x[i][1];
        xold[2] = x[i][2];
        double nx=xoriginal[i][0]/sqrt(xoriginal[i][0]*xoriginal[i][0]+xoriginal[i][1]*xoriginal[i][1]+xoriginal[i][2]*xoriginal[i][2]);
        double ny=xoriginal[i][1]/sqrt(xoriginal[i][0]*xoriginal[i][0]+xoriginal[i][1]*xoriginal[i][1]+xoriginal[i][2]*xoriginal[i][2]);
        double nz=xoriginal[i][2]/sqrt(xoriginal[i][0]*xoriginal[i][0]+xoriginal[i][1]*xoriginal[i][1]+xoriginal[i][2]*xoriginal[i][2]);
        ...
        if (rvarstr && vrvarstr) {
          if (vrvarstyle == EQUAL)
            ...
            {
              v[i][0] = rvelocity[i]*nx;
              v[i][1]= rvelocity[i]*ny;
              v[i][2]= rvelocity[i]*nz;
            }
            
          if (rvarstyle == EQUAL)
          {
            x[i][0] = xoriginal[i][0] + dr*nx;
            x[i][1] = xoriginal[i][1] + dr*ny;
            x[i][2] = xoriginal[i][2] + dr*nz;
          }
          else
          {
            x[i][0] = xoriginal[i][0] + rdisplace[i]*nx;  
            x[i][1] = xoriginal[i][1] + rdisplace[i]*ny;  
            x[i][2] = xoriginal[i][2] + rdisplace[i]*nz;      
          }      
        } else if (rvarstr) {
          if (rvarstyle == EQUAL)
          {
...
void FixMove::final_integrate()
{
  ...
  int rflag=1;
  if (mstyle == SPHERE && (rvarstr || vrvarstr))
    rflag = 0;
...
void FixMove::set_arrays(int i)
{
  ...
  if (mstyle == VARIABLE||mstyle==SPHERE) error->all(FLERR, "Cannot add atoms to fix move variable");
