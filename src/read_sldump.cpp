...
ReadSLDump::ReadSLDump(LAMMPS *lmp) : Command(lmp)
{
...
void ReadSLDump::command(int narg, char **arg)
{
...
void ReadSLDump::store_files(int nstr, char **str)
{
  nfile = nstr;
...
void ReadSLDump::setup_reader(int narg, char **arg)
{
...
bigint ReadSLDump::seek(bigint nrequest, int exact)
{
  int ifile,eofflag;
  bigint ntimestep;
...
bigint ReadSLDump::next(bigint ncurrent, bigint nlast, int nevery, int nskip)
{
  int ifile,eofflag;
  bigint ntimestep;
...
void ReadSLDump::header(int fieldinfo)
{
...
void ReadSLDump::process_atoms()
{
  ...
  // support reading custom attributes
  if(!atom->ivector || !atom->dvector)
    error->all(FLERR,"Atom should set the ivector and dvector before reading!");
  int *tlast=atom->ivector[0];
  int *label=atom->ivector[1];
  double *mindist =atom->dvector[0];
  utils::logmesg(lmp,"READ SLDUMP DEBUG: step {} size of tlast is {}\n",update->ntimestep,sizeof(tlast));
  for (i = 0; i < nnew; i++) {
    ...
    if (replaceflag) {
      ...
      for (ifield = 1; ifield < nfield; ifield++) {
        ...
        case FZ:
          ...
          break;
        case ITLAST:
          tlast[m] = fields[i][ifield];
          break;
        case I2LABEL:
          label[m]= fields[i][ifield];
          break;
        case DMINDIST:
          mindist[m]=fields[i][ifield];
          break;
        }
      }
  ...
  // we shouldn't expect adding new atoms with custom properties (SL)
  if (addflag == NOADD) {
  ...
  for (i = 0; i < nnew; i++) {
    ...
    tlast=atom->ivector[0];
    label=atom->ivector[1];
    mindist =atom->dvector[0];
...
void ReadSLDump::migrate_old_atoms()
{
  tagint *tag = atom->tag;
  int nlocal = atom->nlocal;
...
void ReadSLDump::migrate_new_atoms()
{
  tagint mtag;
  int *procassign;
  double **newfields;
...
void ReadSLDump::migrate_atoms_by_coords()
{
  double **x = atom->x;
  imageint *image = atom->image;
  int nlocal = atom->nlocal;
...
int ReadSLDump::fields_and_keywords(int narg, char **arg)
{
...
int ReadSLDump::whichtype(char *str)
{
  int type = -1;
  ...
  else if (strcmp(str,"i_tlast") == 0) type = ITLAST;
  else if (strcmp(str,"i2_label") == 0) type = I2LABEL;
  else if (strcmp(str,"d_mindist") == 0) type = DMINDIST;
  // adding supprt for SL custom parameters
  // i_tlast i2_label d_mindist
  return type;
}
...
double ReadSLDump::xfield(int i, int j)
{
...
double ReadSLDump::yfield(int i, int j)
{
  if (!scaled) return fields[i][j];
...
double ReadSLDump::zfield(int i, int j)
{
  if (!scaled) return fields[i][j];
